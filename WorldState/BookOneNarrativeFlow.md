...
Generated by `Scripts/build_narrative_flow.py` from `WorldState/BookCodex.json`,
the arc episode map, `WorldState/ContinuityLedger.json` and
`WorldState/AudioProductionBible.json`. Every node label is taken from those files
rather than restated by hand, so this file is a *view* of canon and never an authority
over it. Regenerate rather than hand-edit.

Six left-to-right flowcharts:

1. [The episode spine](#1-the-episode-spine) — every episode, POV, and closing-turn type
...
6. [Protected mysteries](#6-protected-mysteries) — what the arc pays and what it holds
...
Violet nodes are the second viewpoint, blue the protagonist's, gold the arc's
load-bearing turns. The second label line is the mapped closing turn — the state change
the episode is required to deliver.

```mermaid
flowchart LR
  classDef pov1 fill:#eef4fb,stroke:#5588bb,color:#12293d;
  classDef pov2 fill:#f6eff7,stroke:#9a6fa6,color:#33203a;
  ...
  E36 --> E37
  E48 --> E49
  class E01,E02,E03,E04,E05,E06,E07,E08,E09,E10,E11,E13,E14,E15,E16,E17,E19,E20,E22,E23,E25,E26,E27,E28,E30,E32,E33,E34,E35,E36,E38,E39,E40,E41,E43,E44,E46,E47,E48,E50,E51,E52,E53,E54,E56,E58,E59,E60 pov1;
  class E12,E18,E21,E24,E29,E31,E37,E42,E45,E49,E55,E57 pov2;
  class E01,E30,E46,E47,E52,E53,E55,E56,E60 mile;
```
...
The named structural beats, from `season_structure` — these are the gold nodes:
- **Midpoint** — B1E30 — June reads the sealed original in the Trust's vault and understands the Articles are sincere, were drafted to stop a slaughter, and worked. Irreversible: every strategy she had assumed bad faith, and there is no one left to expose.
- **Dark Night** — B1E46 and B1E47 — lawful forfeiture. Milo is taken as surety by scrupulously polite people, Delisle's refusal has closed the vow, and when June offers to answer, Elias refuses to let her. Nothing was done wrong, which is what makes it final.
- **Climax** — B1E53 through B1E55 — in the record, before Delisle as witness to a filing rather than a vow, June reads the Articles back and defeats clauses one, two and three on their own terms against the woman who drafted them. She declines the larger win that would void the peace.
- **Season Resolution** — Three clauses are lawfully defeated and permanently barred from execution; all nine remain visible and the operative count of six is publicly recorded. Milo is released. The price is paid aloud by four named people: Marta loses the business agency, Kenneth loses the Local, Elias is barred from office in perpetuity by his own testimony, and June elects to remain a party to the Articles.
- **Next Season Turn** — The register shows a cut nobody reported. Vera Odell's price was paid by someone, and the deed was done to a person nobody has missed yet.

---

## 2. Plot lines

Each lane is one tracked thread, containing only the episodes where the map records an
action on it. `opened` is the left edge, `resolved` the right. The ceiling is seven
concurrent open threads, of which at most three are protected series mysteries.
...
Non-overlapping four-to-six-episode units. Each answers a local question while advancing
a book thread, which is what keeps the open-thread count under the ceiling.
...
The obligation chain from `ContinuityLedger.json`. Boxes are obligations, labelled with
the episode that creates them and their final recorded status. This is the causal machine
the plot runs on: each clause creates the conditions for the next.

```mermaid
flowchart LR
  classDef live fill:#eef4fb,stroke:#5588bb,color:#12293d;
  classDef closed fill:#eaf3ea,stroke:#4c8a4c,color:#16301a;
  ...
  class OB013,OB014,OB034,OB026,OB027 closed;
  class OB002,OB003,OB011,OB024,OB028,OB029,OB030 perm;
```

The full ledger carries 34 obligations. The ones above are the spine;
the rest are the human-scale costs that hang off it.

---

## 4. Devices

`AudioProductionBible.json` fixes seven reveal channels plus the series signature. Every
essential reveal must arrive as sound, and no channel may run twice in a row. Below, each
channel points at the episodes whose mapped `reveal_channel` it carries.
...
Two prohibitions govern the rotation, from `WritingStyle.md`: never let a silent visual
carry a beat alone, and reserve verbatim repetition for when the repetition itself means
something.
...
flowchart LR
  ...
  N1["eighteen seventy-one<br/><i>execution date of the Articles</i>"]
  N0 --> N1
  N2["three copies<br/><i>sealed original, certified counterpart, parish third</i>"]
  N1 --> N2
  N3["three hundred eleven members<br/><i>what Marta is protecting</i>"]
  N2 --> N3
...
Every episode must end on a state change; the *kind* of turn is deliberately varied so the
audience never learns the rhythm. Distribution across the arc:
...
flowchart LR
  ...
  T5["cliffhanger — 4"]
  H --> T5
  T5_e["E22 · E41 · E45 · E46"]
  T5 --> T5_e
  class T5_e ep;
  T6["decision — 4"]
  H --> T6
  T6_e["E31 · E38 · E49 · E51"]
  T6 --> T6_e
  class T6_e ep;
  T7["emotional turn — 4"]
  H --> T7
  T7_e["E15 · E32 · E33 · E57"]
  T7 --> T7_e
  class T7_e ep;
  T8["open question — 2"]
  H --> T8
...
4 hard cliffhangers in 60 episodes. The machinery the style guide
bans — shock, defusal, fresh shock — is absent by construction.

---

## 6. Protected mysteries

Questions that are never resolved early. The arc pays one instalment of proven fact on
each and holds the answer.
...
flowchart LR
  ...
  P0["The arc pays<br/>The same protective hand created the roster gap and altered the minutes; the released bride confirms that what June's mother took matters before completion. Identity, object, and motive remain unresolved."]
  ...
  P1["The arc pays<br/>Vera establishes the cut trade and both registers reveal one recent unreported cut involving a person neither can name. Buyer and purpose remain unresolved."]
  ...
  E2_t["touched in E12"]
  P2["The arc pays<br/>The Marrow Trust's dependence and nineteen-year scarcity establish measurable institutional pressure. Cause remains unresolved."]
...
| --- | --- |
| Blue rectangle | Protagonist viewpoint, or a live obligation |
| Violet rectangle | Second viewpoint |
...
Regenerate with `Scripts/build_narrative_flow.py`. Canon lives in the WorldState files;
this atlas is a reading of them.
